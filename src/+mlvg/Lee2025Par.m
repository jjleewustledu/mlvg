classdef Lee2025Par < handle & mlvg.Lee2025
    ...
    methods (Static)
        ...
        function [j,c,msg,id] = cluster_build_schaeffer_parc(globbing_mat, opts)
            ...
            arguments
                globbing_mat {mustBeText} = "srcdata_all_delay0.mat"
                opts.globbing_var = ""
                opts.selection_indices double = []  % total ~ 1:58 for ho, 1:69 for co, 1:112 for oo
                opts.Ncol {mustBeInteger} = 16  % Ncol is N of subjects run per job; Nrow is N of sbatch jobs
                opts.account {mustBeTextScalar} = "manu_goyal"
            end
            if isscalar(globbing_mat) && isfile(globbing_mat)
                ld = load(globbing_mat);
                globbed = convertCharsToStrings(ld.(opts.globbing_var));
                globbed = asrow(globbed);
            else
                globbed = asrow(convertCharsToStrings(globbing_mat));
            end
            if isemptytext(opts.globbing_var) && isscalar(globbing_mat)
            ...
            if isscalar(globbing_mat)
                if contains(globbing_mat, "_all", IgnoreCase=true)
                    c = mlvg.CHPC3.propcluster(opts.account, mempercpu='256gb', walltime='24:00:00');
                ...
                c = mlvg.CHPC3.propcluster(opts.account, mempercpu='256gb', walltime='3:00:00');
            end
        ...
        function [j,c,msg,id] = cluster_construct_pet_mipt(globbing_mat, opts)
            ...
            arguments
                globbing_mat {mustBeFile} = ...
                    fullfile(getenv("HOME"), "mnt", "CHPC_scratch", "Singularity", "CCIR_01211", "srcdata_co.mat")
                opts.globbing_var = "srcdata_co"
                opts.minz_for_mip = 5
                opts.minz_for_mip_co = 40
                opts.selection_indices double = []
                opts.Ncol {mustBeInteger} = 5
                opts.out_dir {mustBeTextScalar} = "/scratch/jjlee/Singularity/CCIR_01211"
                opts.account {mustBeTextScalar} = "manu_goyal"
            ...
            % contact cluster slurm
            c = mlvg.CHPC3.propcluster(opts.account, mempercpu='96gb', walltime='24:00:00');
            disp(c.AdditionalProperties)
            for irow = 1:Nrow
                try
                    j = c.batch( ...
                        ...
                        'minz_for_mip_co', opts.minz_for_mip_co, ...
                        'out_dir', opts.out_dir}, ...
                        'Pool', opts.Ncol, ...
                        'CurrentFolder', '/scratch/jjlee/Singularity/CCIR_01211', ...
                        'AutoAddClientPath', false);
        ...
        function [j,c,msg,id] = cluster_deepmrseg_apply(globbing_mat, opts)
            ...
            arguments
                globbing_mat {mustBeFile} = ...
                    fullfile( ...
                    getenv("HOME"), ...
                    "mnt", "CHPC_scratch", "Singularity", "CCIR_01211", "tmp.mat")
                opts.globbing_var = "tmp"
                opts.out_dir {mustBeTextScalar} = "/scratch/jjlee/Singularity/CCIR_01211"
            ...
            ld = load(globbing_mat);
            globbed = convertCharsToStrings(ld.(opts.globbing_var));
            if ~startsWith(globbed(1), opts.out_dir) && ~startsWith(globbed(1), filesep)
        ...
        function durations = par_build_schaeffer_parc(fqfns, opts)
            ...
            if ~contains(fqfns(1), opts.out_dir) && ~startsWith(fqfns(1), filesep)
            ...
            if opts.M == 1
                for fidx = 1:length(fqfns)
                    try
                        ...
                        fqfn = fqfns(fidx);
                        [~,durations(fidx)] = mlvg.Lee2025Par.build_schaeffer_parc(fqfn, out_dir=opts.out_dir);
                    catch ME
                        handwarning(ME)
                    end
                end
        ...
        function durations = par_construct_pet_mipt(nii, opts)
            ...
            parfor (sidx = 1:length(nii), opts.M)
                ...
                try
                    nii_fqfn = fullfile(opts.out_dir, nii(sidx)); %#ok<PFBNS>
                    mlvg.Lee2025.construct_pet_mipt(nii_fqfn, ...
        ...
        function durations = par_copyfile_static(nii, opts)
            ...
            parfor (sidx = 1:length(nii), opts.M)
                ...
                try
                    nii_fqfn = fullfile(opts.out_dir, nii(sidx)); %#ok<PFBNS>
                    mlvg.Lee2025.copyfile_static(nii_fqfn, noclobber=opts.noclobber);
                catch ME
                    handwarning(ME)
                end
            ...
            if 1 == opts.M
                for sidx = 1:length(nii)
                    ...
                    try
                        ...
                        unaligned_fqfn = mglob(strrep(nii_fqfn, "delay0", "unaligned*"));
                        mlvg.Lee2025.time_align([nii_fqfn, unaligned_fqfn], noclobber=opts.noclobber);
                    catch ME
                        handwarning(ME)
                    end
        ...
        function durations = serial_register_t1w_to_t1w(in_fqfns, opts)
            ...
            for idx = 1:numel(in_fqfns)
                ...
                t1w_on_atl_mat = mlvg.Lee2025.register_t1w_to_t1w(in_fqfn, noclobber=true);

                % per T1w, gather all T1w on pets
                sub = extractAfter(extractBefore(in_fqfn, "/ses-"), "derivatives/");
                t1ws_on_pet = [];
                for trc = opts.tracers
